...
const defaultConfig: EnglishSpeedQuizConfig = {
  timeLimitSeconds: 60,
  difficulty: 1,
};
...
function EnglishSpeedChallengeQuizContent() {
  ...
  const quiz = useEnglishSpeedQuiz(config);
  ...
  useEffect(() => {
    if (quiz.state === 'timeUp' && !finished) {
      ...
      addScore('english-speed', { score: accuracy, totalQuestions: quiz.attemptedCount, duration: config.timeLimitSeconds, questionsPerMinute: qpm, date: new Date().toISOString() });

      updateDifficultyProgression('english-speed');
    ...
    return (
      <>
        <EnglishSpeedResult
          ...
          wrongAnswers={quiz.answers.filter((a) => !a.correct)}
          onRetry={handleRetry}
        />
    ...
    return (
      <div className="min-h-screen bg-[#795548] bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] p-4 sm:p-8 font-[var(--font-pixel)] flex items-center justify-center">
        <div className="mc-card bg-[#E2E8F0] p-6 sm:p-12 max-w-lg w-full text-center">
          ...
          <h1 className="text-3xl font-bold text-[#333] mb-2">英语速认挑战</h1>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button onClick={() => { quiz.start(); setStarted(true); play('click'); }} className="mc-btn bg-[#3B82F6] text-white flex-1 py-3">开始挑战</button>
  ...
  return (
    <div className="min-h-screen bg-[#795548] bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] p-4 sm:p-8 font-[var(--font-pixel)]">
      <div className="mb-4 sm:mb-8 flex flex-col sm:flex-row items-center gap-4 max-w-4xl mx-auto">
        <Link href="/english/speed-challenge" className="mc-btn bg-white hover:bg-gray-100 flex items-center gap-2 text-base sm:text-xl w-full sm:w-auto justify-center">
          <ArrowLeft className="h-5 w-5" /> 返回设置
        </Link>
        <h1 className="text-2xl sm:text-3xl text-white drop-shadow-md">英语速认挑战</h1>
        <LanguageSwitcher />
      </div>

      <div className="max-w-4xl mx-auto">
        <div className="mb-6">
          <CountdownTimer secondsRemaining={quiz.timeRemaining} totalSeconds={config.timeLimitSeconds} state={quiz.state} />
        </div>
          ...
          {quiz.currentQuestion ? (
            <>
              <div className="text-center mb-8">
                ...
                <div className="text-4xl sm:text-6xl font-bold text-[#3B82F6]">{quiz.currentQuestion.chinese}</div>
              </div>

              <div className="grid grid-cols-2 gap-4 max-w-lg mx-auto">
                {quiz.currentQuestion.options.map((opt, i) => (
                  <button key={i} onClick={() => handleAnswer(opt)}
                    className={`mc-btn py-6 text-xl sm:text-2xl font-sans transition-colors ${
                      quiz.feedback && quiz.answers.length > 0 && quiz.answers[quiz.answers.length - 1].selectedOption === opt
                        ? quiz.feedback === 'correct' ? 'bg-[#4CAF50] text-white border-black' : 'bg-red-500 text-white border-black'
  ...
  return (
    <div className="min-h-screen bg-[#795548] bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] p-4 sm:p-8 font-[var(--font-pixel)] flex items-center justify-center">
      <div className="mc-card bg-[#E2E8F0] p-6 sm:p-12 max-w-2xl w-full text-center">
        ...
        <h1 className="text-3xl sm:text-4xl font-bold text-[#333] mb-2">{isPerfect ? '完美通关！' : '时间到！'}</h1>
        <p className="text-lg text-gray-600 mb-6">英语速认挑战</p>
              ...
              {wrongAnswers.map((wa, i) => (
                <div key={i} className="bg-red-50 border-2 border-red-200 rounded p-3">
                  <div className="text-sm">
                    <span className="text-xl">{wa.question.chinese}</span>
                  ...
                  <div className="text-sm mt-1">
                    ...
                    <span className="text-green-600 ml-3">正确答案: {wa.question.word}</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <button onClick={onRetry} className="mc-btn bg-[#4CAF50] text-white flex-1 py-3 flex items-center justify-center gap-2">再试一次</button>
          <Link href="/english/speed-challenge" className="mc-btn bg-white text-black flex-1 py-3 flex items-center justify-center gap-2">
            <ArrowLeft className="h-4 w-4" /> 返回设置
          </Link>
        </div>
