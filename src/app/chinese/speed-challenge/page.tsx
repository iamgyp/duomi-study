...
export default function ChineseSpeedChallengePage() {
  const router = useRouter();
  const [timeLimit, setTimeLimit] = useState<30 | 60 | 120>(60);
  const [difficulty, setDifficulty] = useState<1 | 2 | 3>(1);

  const handleStart = () => {
    router.push(`/chinese/speed-challenge/quiz?timeLimit=${timeLimit}&difficulty=${difficulty}`);
  };

  const timeLabel = `${timeLimit}秒`;
    ...
    <div className="min-h-screen bg-[#795548] bg-[url('https://www.transparenttextures.com/patterns/wood-pattern.png')] p-4 sm:p-8 font-[var(--font-pixel)]">
      <div className="mb-4 sm:mb-8 flex flex-col sm:flex-row items-center gap-4 max-w-7xl mx-auto">
        ...
        <div className="flex items-center gap-2 sm:gap-4 bg-black/40 px-4 sm:px-6 py-2 rounded-sm border-2 border-white/20 backdrop-blur-sm">
          ...
          <h1 className="text-2xl sm:text-4xl text-white drop-shadow-md tracking-wider">汉字速认挑战</h1>
        </div>
        ...
        <div className="lg:col-span-1">
          <div className="mc-card bg-[#E2E8F0] p-6">
            <div className="space-y-6 font-sans">
                ...
                <div className="space-y-2">
                  {[1, 2, 3].map((level) => (
                    <button
                      ...
                      className={`w-full border-2 border-black py-2 text-sm font-bold transition-all active:translate-y-1 ${
                        difficulty === level
                          ? 'bg-yellow-400 text-black'
                          : 'bg-white text-gray-600 hover:bg-gray-100'
                      }`}
                    >
                      {level === 1 ? '⭐ 基础（常用字）' : level === 2 ? '⭐⭐ 进阶（形容词）' : '⭐⭐⭐ 挑战（高级字）'}
                    </button>
                  ))}
                </div>
              </div>
              ...
              <button
                onClick={handleStart}
                className="mc-btn w-full bg-[#F59E0B] text-white text-lg hover:bg-[#D97706] flex items-center justify-center gap-2 py-3"
              >
